...
## Visualize the Time Series Object
...
```{r}
fnb_pad %>% 
  mutate(seasonal = fnb_dec$seasonal,
         hour = hour(time)) %>% 
...
fnb_msts <- msts(data = fnb_pad$visitor, seasonal.periods = c(24, 24*7))
...
as.data.frame(fnb_msts_dec) %>% 
  mutate(day = fnb_pad$day,
         hour = as.factor(hour(fnb_pad$time))) %>% 
  group_by(day, hour) %>% 
